...
function ourExcept({ range = [], list = [] }) {
  var exceptRange = rangeBuilder('every', function (left, right) { 
    return right.start > left || right.end < left; 
  });
  
  var exceptList = rangeBuilder('every', function (left, right) { 
    return right.start === left || right.end === left; 
  });
  
  var exceptFuncs = range.map(r => exceptRange(...r)).concat(list.map(l => exceptList(l, l)));

  return function(date) {
    return exceptFuncs.some(func => func(date));
  };
}
...
// TODO : Hem aralık hem de liste halinde exception verilebilir hale getirilecek
var ourExcept = ourExcept({
  range: [['2014-08-09', '2014-09-01'], ['2023-01-01', '2024-01-01']],
  list: ['2024-02-01', '2024-03-01']
});
