class ProdseenSelect {
    ...
    constructor(
        ...
        customSelect.classList.add(`prodseen-select-${this.#uts}`);

        this.#multipleselect = document.createElement('div');
        this.#multipleselect.classList.add(`prodseen-multiple-select-${this.#uts}`);
        customSelect.appendChild(this.#multipleselect);

        this.#searchInput = document.createElement("input");
        this.#searchInput.type = 'text';
        this.#searchInput.classList.add(`prodseen-select-search-${this.#uts}`);
        ...
        this.#optionsList.classList.add(`prodseen-select-options-${this.#uts}`);
    ...
    #addEventListeners(element) {
        ...
        document.addEventListener("click", (event) => {
            if (!element.contains(event.target) && (event.target.classList[0] != `prodseen-item-${this.#uts}` && event.target.classList[1] != `prodseen-item-${this.#uts}`)) {
    ...
    #set_selected(text, value, image, index) {
        ...
        const span = document.createElement("span");
        span.classList.add(`prodseen-multiple-selected-${this.#uts}`);
        span.classList.add(`prodseen-item-${this.#uts}`);
        let imagesrc;
    ...
    #validate_options(options) {
        var state = true;
        if (!Array.isArray(options)) {
            console.error("ProdseenSelect : Options must be an Array!");
            return false;
        }
    ...
    #validate_option(object, i) {
        ...
        if (!option.hasOwnProperty('text') || !option.hasOwnProperty('value')) {
            console.error(`ProdseenSelect: Text and value properties must be strings at index ${i}.`);
        ...
        try {
            if (option.hasOwnProperty('image')) {
                if (typeof option.image !== 'string') {
                    console.error(`ProdseenSelect: Invalid image path at index ${i}.`);
                    return false;
                }
            }
        } catch (err) {
    ...
    #add_options() {
        ...
        for (let i = 0; i < this.#options.length; i++) {
            ...
            const li = document.createElement('li');
            li.classList.add(`prodseen-item-${this.#uts}`);
            const img = document.createElement('img');
            img.classList.add(`prodseen-item-${this.#uts}`);
    ...
    #set_stylesheet() {
        ...
        .prodseen-multiple-select-${this.#uts}{
            overflow-x: auto;
            white-space: nowrap;
        }

        .prodseen-multiple-selected-${this.#uts}{
        ...
        .prodseen-multiple-selected-${this.#uts}:hover{
            background-color:#AAA;
        }

        .prodseen-multiple-selected-${this.#uts} img{
            position:relative;
            margin:0;
            padding:0;
            height:24px;
          ...
          .prodseen-select-search-${this.#uts} {
            font-family: 'Poppins', sans-serif;
            font-size:14px;
            width:99%;
            padding:0;
          ...
          .prodseen-select-options-${this.#uts} {
            list-style: none;
            padding: 2px;
            margin: 0;
            position: relative;
          ...
          .prodseen-select-options-${this.#uts} li {
            height:24px;
            padding: 4px;
            cursor: pointer;
            font-family: 'Poppins', sans-serif;
          ...
          .prodseen-select-options-${this.#uts} li > img {
            margin-right:4px;
            font-family: 'Poppins', sans-serif;
            font-size:14px;
          }
        
          .prodseen-select-options-${this.#uts} li:hover {
...
const options = [
    ...
    { text: 'Prodseen', value: "Fed test", image: 'assets/img/icon.png' },
];

const prodseenSelect = new ProdseenSelect("#select_test",
    {
        width: "400px",
        placeholder: "Arama",
        search: true
